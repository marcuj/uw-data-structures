        ...
        private final Node source = new Node() {
            ...
            public List<Edge<Node>> neighbors(Picture picture, EnergyFunction f) {
                ...
                for (int y = 0; y < picture.height(); y += 1) {
                    result.add(new Edge<>(this, new Pixel(0, y), f.apply(picture, 0, y)));
                }
                return result;
            }
        };
        ...
        public class Pixel implements Node {
            ...
            public List<Edge<Node>> neighbors(Picture picture, EnergyFunction f) {
                ...
                if (x == picture.width() - 1) {
                    ...
                    for (int i = 0; i < 3; i++) {
                        int neighborY = y - 1 + i;
                        if (0 <= neighborY && neighborY < picture.height()) {
                            Pixel p = new Pixel (x + 1, neighborY);
                            neighbors.add(new Edge<>(this, p, f.apply(picture, x + 1, neighborY)));
                        }
                    }
                }
                return neighbors;
